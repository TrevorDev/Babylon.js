module BABYLON {
    ...
    export class BloomEffect extends PostProcessRenderEffect{
        ...
        constructor(scene: Scene, bloomScale:number, bloomKernel:number, pipelineTextureType = 0, blockCompilation = false) {
            ...
            var copyBack = new PassPostProcess("bloomBlendBlit", bloomScale, null, Texture.BILINEAR_SAMPLINGMODE, scene.getEngine(), false, pipelineTextureType);
            copyBack.alwaysForcePOT = true;

            this._effects = [blurX, blurY]
        }
    }
}