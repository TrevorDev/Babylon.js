                ...
                this.scene.getEngine()._onBeforeRenderObservable.add((renderInfo)=>{
                    ...
                    if(pose){
                        ...
                        renderInfo.frame.views.forEach((view:any, i:number)=> {
                            ...
                            this._camera.rigCameras[i].viewport.width = viewport.width// /this._webXRSession.baseLayer.framebufferWidth;
                            this._camera.rigCameras[i].viewport.height = viewport.height// /this._webXRSession.baseLayer.framebufferHeight;
                            this._camera.rigCameras[i].viewport.x = viewport.x // /this._webXRSession.baseLayer.framebufferWidth;
                            this._camera.rigCameras[i].viewport.y = viewport.y// /this._webXRSession.baseLayer.framebufferHeight;
                            this._camera.rigCameras[i].getViewMatrix()
                            this._camera.rigCameras[i]._outputBuffer = this._xrSession.baseLayer.framebuffer;


                            // var mat = BABYLON.Matrix.FromArray(pose.getViewMatrix(view))
                            // mat.invert()
                            ...
                            //     });
                            //     // [8, 9, 10, 11].forEach((num) => {
                            //     //     mat._projectionMatrix.m[num] *= -1;
                            //     // });
                            // }
                            
                            // // camera.position.copyFrom(mat.getTranslation())
                            // // camera.rotationQuaternion = new BABYLON.Quaternion()
                            // // camera.rotationQuaternion.fromRotationMatrix(mat.getRotationMatrix())
                            // console.log("pose")
                        })
                    }
                });
